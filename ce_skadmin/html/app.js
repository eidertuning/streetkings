(function(){
  ...
  function dashboard(p){
    ...
    const target=currentSave(p)||{};
    return `<div class="admin-dashboard"><section class="control-hero admin-hero"><div><div class="control-hero-kicker">${icon('shield')} Five Horizon</div><h2>Sala de control</h2></div><div class="hero-actions"><button class="btn gold" data-tab="actions">Acciones</button><button class="btn" data-tab="screen">Vista</button></div></section><div class="admin-metrics"><div class="metric-card"><span>Jugadores</span><b>${players.length}</b></div><div class="metric-card"><span>Activos</span><b>${active}</b></div><div class="metric-card"><span>Objetivo</span><b>${p?'#'+p.id:'--'}</b></div><div class="metric-card"><span>Slot</span><b>${target.slotIndex?target.slotIndex:'--'}</b></div></div><section class="room-board card clean-board"><div class="room-board-head"><h3>Sala admin</h3><span>${players.length} entradas</span></div><div class="room-list-grid">${roomCards}</div></section></div>`;
  }
  function actions(p){
    const groups=[
      ['Jugador',[['player.goto','location','Ir'],['player.bring','hand','Traer'],['player.spectate','eye','Observar'],['capture.open','camera','Vista'],['player.freeze','snow','Congelar'],['player.revive','heart','Revivir'],['player.heal','heart','Curar'],['player.armor','shield','Chaleco'],['player.kill','skull','Matar']]],
      ['Vehículo',[['vehicle.repair','wrench','Reparar'],['vehicle.clean','soap','Limpiar'],['vehicle.flip','rotate','Voltear'],['vehicle.warpwp','map','Waypoint']]],
      ['Sistema',[['vehicle.speedometer','gauge','Velocímetro'],['vehicle.soundtrack','music','Música'],['vehicle.cinematic','video','Cine'],['phone.toggle','phone','Teléfono']]]
    ];
    return `<div class="action-board refined-actions">${groups.map(g=>`<section class="card action-section"><div class="action-section-head"><h3>${icon('bolt')} ${g[0]}</h3><span>${g[1].length}</span></div><div class="action-grid">${g[1].map(a=>`<button class="action-btn compact-action" data-action="${a[0]}" ${!p?'disabled':''}>${icon(a[1])}<span>${a[2]}</span></button>`).join('')}</div></section>`).join('')}</div>`;
  }
  ...
  function world(){
    ...
    const label=labels[weather]||weather;
    const meta=`${current.autoWeather===false?'Auto OFF':'Auto ON'} · ${current.weatherFrozen?'Fijado':'Libre'}`;
    return `<div class="world-clean enhanced-world pro-world"><section class="weather-showcase card">${skyScene(current)}<div class="weather-overlay"><span>Ambiente</span><b>${hh}:${mm}</b><strong>${esc(label)}</strong><small>${esc(meta)}</small></div></section><div class="world-controls"><section class="card section"><div class="card-head compact-head"><h3>${icon('world')} Hora</h3><span>${hh}:${mm}</span></div><div class="chip-grid time-grid">${TIMES.map(h=>`<button class="chip time-chip ${(Number(current.h)===h)?'active':''}" data-world-time="${h}"><span>${String(h).padStart(2,'0')}:00</span><small>${h>=20||h<6?'Noche':h<12?'Mañana':h<18?'Día':'Tarde'}</small></button>`).join('')}</div><label class="amount-row custom-hour-row"><span>Hora personalizada</span><div><input id="customHour" type="number" min="0" max="23" placeholder="0-23"><button class="btn gold" data-custom-time>Aplicar</button></div></label></section><section class="card section"><div class="card-head compact-head"><h3>${icon('world')} Clima</h3><span>${esc(label)}</span></div><div class="chip-grid weather-grid animated-weather">${WEATHER.map(w=>weatherCard(w,labels[w]||w,hNum)).join('')}</div></section></div><div class="card section siren-card clean-siren"><div><h3>${icon('music')} Sirena de tsunami</h3></div><div class="siren-actions"><button class="btn gold" data-siren-test>Probar</button><button class="btn danger" data-siren-stop>Detener</button></div></div></div>`;
  }
